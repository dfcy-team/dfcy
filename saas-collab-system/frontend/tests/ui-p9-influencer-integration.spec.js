...
describe('influencer integration workspace contracts', () => {
  ...
  it('registers outreach and fulfillment routes and exposes permission-scoped menu entries', () => {
    ...
    expect(performanceMenu.permissions).toEqual(['influencers.view']);
    ...
    const readUser = { user_type: 'internal', permissions: ['influencers.view'] };
    const paths = flattenMenuItems(filterMenuItems(readUser)).map((item) => item.path);
    expect(paths).toContain('/influencers');
    expect(paths).toContain('/influencers/bd-performance');
    expect(canAccessPath(readUser, '/influencers/bd-performance')).toBe(true);
