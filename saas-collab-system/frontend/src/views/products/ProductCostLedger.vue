<template>
  <section class="cost-page">
    <header class="page-header">
      <div>
        <h1>商品成本</h1>
        <p>按生效期间维护 SKU 成本版本并核对系统生成结果；采购价格仅作为成本构成项，历史成本永久保留。</p>
      </div>
    ...
    <el-alert
      ...
      title="成本口径：商品成本 = 采购价格 + 物流分摊 + 税费 + 包装费 + 其他费用。调整成本只新增版本，不覆盖历史；下游业务按发生时间锁定成本快照。"
      type="info"
      :closable="false"
      show-icon
    />
    ...
    <div class="summary-strip" aria-label="成本核对概览">
      <div><span>SKU 数量</span><strong>{{ rows.length }}</strong><small>当前查询范围</small></div>
      ...
      <el-table v-loading="loading" :data="visibleRows" row-key="id" border empty-text="暂无商品成本数据">
        ...
        <el-table-column label="操作" width="210" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="canManage" link type="primary" @click="edit(row)">维护</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-drawer v-model="drawerVisible" :title="drawerReadonly ? '成本构成' : '维护商品成本'" size="480px">
      ...
      <el-form label-position="top">
        ...
        <el-form-item v-if="!drawerReadonly" label="新版本生效日期" required>
    ...
    <el-dialog v-model="historyVisible" title="成本历史版本" width="min(720px, 94vw)">
      <p class="history-note">{{ historySku }} · 已结束版本仍用于还原历史订单、利润及经营报表，不随新版本变化。</p>
...
const applied = reactive({ search: '', status: '' });
...
const visibleRows = computed(() => rows.value.filter((row) => {
  ...
  return matchesSearch && (!applied.status || row.status === applied.status);
}));
...
const statusCount = (status) => rows.value.filter((row) => row.status === status).length;
...
async function load() {
  loading.value = true;
  const response = await fetchProductCosts();
  loading.value = false;
  ...
  versions.forEach((version) => {
    const item = grouped.get(version.sku) || { id: version.sku, sku_id: version.sku, sku_code: version.sku_code, product_name: version.product_name, versions: [] };
    item.versions.push({ ...version, cost: version.confirmed_cost, source: version.source === 'manual' ? '人工维护' : '系统生成' });
    grouped.set(version.sku, item);
  });
...
function resetFilters() { Object.assign(filters, { search: '', status: '' }); applyFilters(); }
...
function showHistory(row) { historySku.value = row.sku_code; historyRows.value = row.versions || []; historyVisible.value = true; }
...
async function save() {
  ...
  if (!form.effective_from) return ElMessage.warning('请选择新版本生效日期');
  ...
  if (index >= 0) {
    ...
    const nextVersion = `V${Number(String(previous.version_no || 'V0').replace('V', '')) + 1}`;
    const archived = previous.confirmed_cost === null ? [] : [{ version_no: previous.version_no, cost: previous.confirmed_cost, effective_from: previous.effective_from, effective_to: form.effective_from, source: previous.source === 'manual' ? '人工维护' : '系统生成' }, ...(previous.versions || []).slice(1)];
    rows.value[index] = { ...previous, ...payload, version_no: nextVersion, versions: [{ version_no: nextVersion, cost: payload.confirmed_cost, effective_from: form.effective_from, effective_to: null, source: '人工维护' }, ...archived], system_cost: calculatedFormCost.value.replace('¥', ''), source: 'manual' };
  }
