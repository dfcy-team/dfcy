  ...
  <section class="resource-library">
    ...
    <el-card class="workspace-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" empty-text="暂无达人档案" @row-click="openDetail">
        <el-table-column label="达人" min-width="190"><template #default="{ row }"><b>{{ influencerDisplayName(row) }}</b><small>{{ displayValue(row.code) }} · {{ displayValue(row.display_name) }}</small></template></el-table-column>
    ...
    <el-dialog v-model="editVisible" :title="editing ? '编辑达人档案' : '新建达人档案'" width="760px" append-to-body :close-on-click-modal="false" @closed="resetForm">
      <el-form label-position="top"><div class="form-grid">
        ...
        <el-form-item label="账号"><el-input v-model="form.handle" :disabled="Boolean(editing)" placeholder="仅新建或账号解析时录入" /></el-form-item>
    ...
    <el-drawer v-model="detailVisible" :title="detail?.name || '达人详情'" size="560px">
      ...
      <template v-else-if="detail">
        ...
        <el-descriptions :column="2" border><el-descriptions-item label="达人名称">{{ displayValue(detail.name) }}</el-descriptions-item><el-descriptions-item label="平台">{{ displayValue(detail.platform) }}</el-descriptions-item><el-descriptions-item label="TikTok用户名">{{ displayValue(detail.handle) }}</el-descriptions-item><el-descriptions-item label="达人编码">{{ displayValue(detail.code) }}</el-descriptions-item><el-descriptions-item label="展示名称">{{ displayValue(detail.display_name || detail.profile?.display_name) }}</el-descriptions-item><el-descriptions-item label="内容赛道">{{ displayValue(detail.category) }}</el-descriptions-item><el-descriptions-item label="粉丝数">{{ formatCount(detail.follower_count) }}</el-descriptions-item><el-descriptions-item label="合作分层">{{ cooperationLabel(detail.cooperation_status) }}</el-descriptions-item><el-descriptions-item label="市场">{{ displayValue(detail.profile?.market) }}</el-descriptions-item><el-descriptions-item label="档案链接">{{ displayValue(detail.profile?.profile_url) }}</el-descriptions-item><el-descriptions-item label="档案状态">{{ detail.status === 'active' ? '正常' : '停用' }}</el-descriptions-item><el-descriptions-item label="创建时间">{{ formatTime(detail.created_at) }}</el-descriptions-item><el-descriptions-item label="更新时间">{{ formatTime(detail.updated_at) }}</el-descriptions-item></el-descriptions>
...
const influencerDisplayName = (row) => row?.handle || row?.display_name || row?.profile?.display_name || row?.name || row?.code || '—';
...
async function openEdit(row) {
  ...
  const [profileResponse, contactsResponse] = await Promise.all([fetchInfluencer(row.id), fetchInfluencerContacts(row.id)]);
  if (!profileResponse?.success) return ElMessage.error(profileResponse?.message || '达人档案加载失败，请重试');
  const fullProfile = profileResponse.data || {};
  const editable = {
    ...row,
    ...fullProfile,
    contacts: collectionRows(contactsResponse?.data || fullProfile.contacts || row.contacts || [])
  };
  editing.value = editable;
  Object.assign(form, {
    ...blankForm(),
    ...editable,
    profile: profileForm(editable.profile),
    contacts: editable.contacts.map((contact) => ({ ...contact, key: contact.id || `${contact.channel}-${contact.value}` }))
  });
