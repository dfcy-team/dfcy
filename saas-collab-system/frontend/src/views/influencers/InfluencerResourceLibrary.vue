  ...
  <section class="resource-library">
    ...
    <el-card class="workspace-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" :empty-text="listError ? '达人档案加载失败，请重试' : '暂无达人档案'" @row-click="openDetail">
        <el-table-column label="达人账号" min-width="200" fixed="left"><template #default="{ row }"><b>{{ influencerDisplayName(row) }}</b><small>{{ profileValue(row, 'external_influencer_id') }} · {{ displayValue(row.platform) }}</small></template></el-table-column>
