<template>
  <section class="creator-page">
    ...
    <el-dialog v-model="createVisible" title="新建建联任务" width="580px"><el-form label-width="110px">
      ...
      <el-form-item label="商品 ID"><el-input v-model="form.external_product_id" @change="matchProduct"/></el-form-item><el-form-item v-if="productMatchHint" label="匹配结果"><el-alert :closable="false" :type="productMatchType" :title="productMatchHint"/></el-form-item><el-form-item label="SKU 前缀"><el-select v-if="matchedSkuPrefixes.length>1" v-model="form.sku_prefix" placeholder="请选择 SKU 前缀"><el-option v-for="prefix in matchedSkuPrefixes" :key="prefix" :label="prefix" :value="prefix"/></el-select><el-input v-else v-model="form.sku_prefix"/></el-form-item><el-form-item label="目标建联人数" required><el-input-number v-model="form.target_count" :min="1" :step="1" step-strictly/></el-form-item>
...
import { addOutreachTarget,createOutreachTask,deleteOutreachTarget,fetchOutreachTargets,fetchOutreachTaskOptions,fetchOutreachTasks,formatInfluencerError,matchOutreachProduct,OUTREACH_RESULT_LABELS,OUTREACH_STATUS_LABELS,restoreOutreachTarget,statusLabel,updateOutreachStatus,updateOutreachTarget } from '../../api/influencers';
