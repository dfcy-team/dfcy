<template>
  <section class="creator-page">
    ...
    <el-card shadow="never"><el-table v-loading="loading" :data="rows" empty-text="暂无建联任务">
      ...
      <el-table-column label="操作" width="160"><template #default="{row}"><el-button text :disabled="!canManage||row.status!=='pending'" @click="changeStatus(row,'in_progress')">开始</el-button><el-button text :disabled="!canManage||['completed','cancelled'].includes(row.status)" @click="changeStatus(row,'completed')">完成</el-button></template></el-table-column>
    </el-table></el-card>
...
<script setup>
import { computed,onMounted,reactive,ref } from 'vue';import{ElMessage}from'element-plus';import{useAuthStore}from'../../stores/auth';import{createOutreachTask,fetchOutreachTasks,updateOutreachStatus}from'../../api/influencers';
const auth=useAuthStore(),rows=ref([]),loading=ref(false),saving=ref(false),dialog=ref(false);const form=reactive({task_no:'',influencer:1,store:1,owner:1});const canManage=computed(()=>auth.hasPermission('influencers.outreach.manage'));
async function load(){loading.value=true;const r=await fetchOutreachTasks();loading.value=false;if(r.success)rows.value=r.data.results||[];else ElMessage.error(r.message)}
