<template>
  <AppPage eyebrow="PRODUCTION PILOT" title="容量观察" subtitle="查看受控环境回传的容量证据和有效期。" boundary-note="容量数据用于准入判断；扩缩容仍由独立基础设施审批流程执行。" :capability="capability">
...
const state = ref('loading'); const capability = ref('connected'); const errorMessage = ref(''); const data = ref({ metrics: [] });
async function load() { state.value = 'loading'; const response = await fetchCapacitySummary({ environment_id: 'controlled-pilot', window_minutes: 15 }); if (!response.success) { state.value = statusFromApiResponse(response, typeof navigator === 'undefined' || navigator.onLine); errorMessage.value = response.message; return; } data.value = response.data; capability.value = response.data?.api_status || 'connected'; state.value = 'ready'; }
load();
</script>
