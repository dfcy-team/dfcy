  ...
  <section class="sales-workspace" :aria-busy="loading">
    <header class="sales-header">
      ...
      <div class="sales-header__actions">
        ...
        <el-button v-if="canExport" @click="openExportDialog">按当前筛选申请导出</el-button>
      </div>
    </header>
    ...
    <el-form v-if="resolvedFilters.length" class="sales-filters" :model="query" label-position="top" @submit.prevent="applyFilters">
    ...
    <div v-loading="pageState === 'loading'" class="sales-content">
      ...
      <div v-if="mode === 'overview' && trend.length" class="overview-grid">
        ...
        <section class="sales-panel anomaly-panel">
          ...
          <button v-for="issue in anomalies" :key="issue.id" type="button" class="anomaly-row" @click="goToIntegrations">
        ...
        <el-table v-if="rows.length" :data="rows" stripe @row-click="selectRow">
          ...
          >
            <template #default="{ row }">
              ...
              <span v-else>{{ displayValue(valueAt(row, column.prop)) }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="mode === 'orders'" label="操作" width="130" fixed="right">
            <template #default="{ row }">
              ...
              <el-button text @click.stop="copyReference(row.external_order_id)">复制</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-drawer v-model="detailOpen" size="520px" title="只读详情">
      <dl v-if="selectedRow" v-loading="detailLoading" class="detail-list">
        <div v-for="column in contract.columns" :key="column.prop"><dt>{{ column.label }}</dt><dd>{{ displayValue(valueAt(selectedRow, column.prop)) }}</dd></div>
      </dl>
      <section v-if="selectedRow?.items?.length" class="detail-lines">
        ...
        <el-table :data="selectedRow.items" size="small">
          ...
          <el-table-column prop="original_unit_price" label="原价" width="110" align="right" />
          <el-table-column prop="sale_unit_price" label="折后价" width="110" align="right" />
          <el-table-column prop="discount_amount" label="折扣" width="110" align="right" />
          <el-table-column prop="line_total_amount" label="行金额" width="110" align="right" />
        </el-table>
      </section>
      <section v-if="selectedRow?.refund_returns?.length" class="detail-lines">
        ...
        <article v-for="refund in selectedRow.refund_returns" :key="refund.id" class="refund-card">
          <dl class="detail-list">
            ...
            <div><dt>类型 / 状态</dt><dd>{{ displayValue(refund.case_type) }} / {{ displayValue(refund.normalized_status) }}</dd></div>
            <div><dt>退款金额</dt><dd>{{ refund.currency }} {{ displayValue(refund.refund_amount) }}</dd></div>
            <div><dt>费用拆分</dt><dd>商品 {{ displayValue(refund.refund_subtotal) }} · 运费 {{ displayValue(refund.refund_shipping_fee) }} · 税 {{ displayValue(refund.refund_tax) }}</dd></div>
          ...
          <el-table v-if="refund.items?.length" :data="refund.items" size="small">
            ...
            <el-table-column prop="refund_amount" label="退款金额" width="110" align="right" />
          </el-table>
        </article>
      </section>
  ...
  try {
    ...
    metrics.value = data.metrics?.length
      ...
      : (data.currency_groups || []).flatMap((group) =>
          (group.metrics || []).map((metric) => ({ ...metric, code: `${group.currency}-${metric.code}`, label: `${group.currency} · ${metric.label}` }))
        );
    trend.value = (data.trend || []).map((point) => ({
      ...point,
      label: point.label || point.date,
      value: point.value ?? point.net_sales
    }));
...
watch(() => props.mode, () => { initializeFilters(); loadFilterOptions(); loadData(); }, { immediate: true });
void UI_STATES;
</script>
