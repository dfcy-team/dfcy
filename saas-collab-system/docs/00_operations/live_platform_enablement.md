...
3. `LIVE_CUSTODY_BACKEND=file|http`，并配置批准的业务库外 custody；
4. callback result 使用无 query/fragment 的精确 HTTPS allowlist 页面；
5. `DEBUG=false`、平台/custody host allowlist 非空、平台合同批准开关为真。
...
Platform Console
  ...
  -> Approved Credential Custody（本地 0700 独立目录或 HTTPS 服务）
  -> opaque app-secret / credential / token reference
  -> Application Runtime
```

禁止开发人员通过聊天、普通文本、`.env`、SQL、日志或 PR 中转 secret。应用数据库只保存引用、掩码、版本、状态、时间戳、scope 与受控错误码。本地文件 custody 只能使用 Git/业务数据库之外的 operator-owned 目录，目录模式 `0700`、记录模式 `0600`，以原子替换和跨进程锁维护；不得放在项目目录内。
...
- OAuth：精确 callback allowlist；无 query 的结果页 allowlist；平台公开 app/service identifier；App Secret 仅配置 custody reference。
