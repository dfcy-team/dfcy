...
当前允许继续受控内网试点验证，但尚不建议宣布正式生产准入：目标主机使用本次复验生成的短期自签名证书，尚未替换为组织批准并由目标客户端信任的正式证书；两台主机均提示 1 项标准安全更新待处理；本轮未使用独立试点业务账号完成浏览器认证态 E2E；虚拟机登录凭据曾通过协作会话明文提供，必须在正式放行前轮换。上述事项不得被当前技术复验替代。
...
| PILOT-ENTRY-R2-P1-003 | 虚拟机登录凭据曾在协作会话中明文提供，且当前口令不满足正式环境强度 | 凭据暴露与弱口令会削弱主机边界，不能作为正式试点长期凭据 | 轮换应用和数据库虚拟机口令；优先使用独立 SSH 密钥并关闭密码登录；确认旧凭据失效并复验运维登录与 sudo 边界 |
...
PR、CI 和目标虚拟机复验流程已完成，原 3 项技术 P1 已关闭。当前结论保持 `CONDITIONAL_PASS`：可以继续受控内网验证，但需完成正式证书、认证态 E2E、主机安全更新和登录凭据轮换后，才建议宣布生产试点正式准入。
