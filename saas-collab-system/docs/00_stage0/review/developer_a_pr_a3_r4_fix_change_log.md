...
New Code Review SHA: 4c75eda5458fafc9d1d9fb0833392c94ce106f2a
...
独立复审随后在 Evidence HEAD `d52f11f8994ed7291baa67d5469c2b390787532c` 发现两项追加问题：P1 为 Django 自动生成的普通 `_base_manager.bulk_create()` 仍能绕过自定义默认 manager；P2 为 PR 正文仍冻结在 R3 SHA 和旧测试结果。追加整改入场状态为 `Open P1: 1`、`Open P2: 1`、`Independent R4 Review: FAIL / REQUEST CHANGES`。
...
- 在 attempt 模型 `Meta` 中将既有受保护的 `objects` 指定为 `base_manager_name`，使 `_base_manager` 复用同一拒绝策略；没有新增 manager、全局写开关或其他受控创建入口。
- 新增 `_base_manager.bulk_create()` 负向测试，证明整改前可复现写入、整改后抛出受控错误且数据库保持 0 条。
- 新增 `marketplace_imports.0003`，仅记录 Django base manager 模型元数据，不修改表、列、索引或约束；fresh 和 `0001 -> 0002 -> 0003` upgrade 均通过。

没有修改公共权限、统一异常、data scope、settings 或路由。必要的 `0003` 只影响 `MarketplaceImportBatchAttempt` manager 选择，范围不扩散到其他模型。
...
| 21 | `_base_manager.bulk_create` 拒绝且不写入 | PASS |
...
| SQLite focused | PASS，56 passed / 1 MySQL-only skipped |
| SQLite backend full | PASS，599 passed / 4 MySQL-only skipped |
| MySQL version | PASS，8.4.11 Community Server |
| MySQL fresh migration | PASS，迁移至 head |
| MySQL upgrade | PASS，`marketplace_imports.0001 -> 0002 -> 0003` |
| MySQL migration drift | PASS，No changes detected |
| MySQL focused | PASS，57 passed / 0 skipped |
...
本次独立复审追加 Code Review SHA：

- `backend/apps/marketplace_imports/models.py`
- `backend/apps/marketplace_imports/migrations/0003_alter_marketplaceimportbatchattempt_options.py`
...
本次追加整改修改 3 个代码/测试文件和 4 个 evidence-only 文档；Evidence HEAD 相对 Base 的最终统计在移交时确认。
