...
2. 对未获准 renderer/layout 使用明确空集合和阻断状态，不得提前填入已授权版本；
3. 保留源 commit、源文件、OFL、派生脚本和每个候选的完整摘要；
4. 更新 manifest 后重新计算其字节数和 SHA-256；
5. 因 manifest 是候选包组成部分，修订后的精确四文件集合必须重新快速复核。
