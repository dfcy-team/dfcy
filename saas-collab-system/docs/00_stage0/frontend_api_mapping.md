...
| 平台账单列表 | `/finance/statements` | `/api/finance/statements/` | GET | `page`、`page_size`、`platform`、`currency`、`status` | `count/next/previous/results`及账单字段 | `frontend/src/mock/financeReconciliation.js` | pending（待模块JWT复验） |
| 提现记录列表（只读） | `/finance/withdrawals` | `/api/finance/withdrawals/` | GET | `page`、`page_size`、`platform`、`currency`、`status` | `count/next/previous/results`及提现记录字段 | `frontend/src/mock/financeReconciliation.js` | pending（不执行提现） |
| 银行到账列表 | `/finance/bank-receipts` | `/api/finance/bank-receipts/` | GET | `page`、`page_size`、`platform`、`currency`、`status` | `count/next/previous/results`；账号仅返回`masked_account` | `frontend/src/mock/financeReconciliation.js` | pending（待模块JWT复验） |
| 对账匹配列表 | `/finance/reconciliation/matches` | `/api/finance/reconciliation/matches/` | GET | `page`、`page_size`、`platform`、`currency`、`status` | `count/next/previous/results`及匹配建议字段 | `frontend/src/mock/financeReconciliation.js` | pending（待模块JWT复验） |
| 对账匹配详情 | `/finance/reconciliation/matches/{id}` | `/api/finance/reconciliation/matches/{id}/` | GET | `id` | 脱敏匹配建议详情 | `frontend/src/mock/financeReconciliation.js` | pending（待模块JWT复验） |
| 对账匹配动作 | 同上 | `/api/finance/reconciliation/run-mock/`、`matches/{id}/confirm/`、`matches/{id}/reject/` | POST | 合成平台/币种、`Idempotency-Key`、人工原因 | 建议状态与不可变审计；不执行资金操作 | `frontend/src/mock/financeReconciliation.js` | pending；run-mock为synthetic |
| 对账异常列表 | `/finance/reconciliation/exceptions` | `/api/finance/reconciliation/exceptions/` | GET | `page`、`page_size`、`platform`、`currency`、`status` | `count/next/previous/results`及异常字段 | `frontend/src/mock/financeReconciliation.js` | pending（待模块JWT复验） |
| 对账异常处理 | 同上 | `/api/finance/reconciliation/exceptions/{id}/resolve/` | POST | `resolution_note` | 已解决状态与不可变审计；不执行资金操作 | `frontend/src/mock/financeReconciliation.js` | pending（需`finance.exception.handle`） |
...
| 财务对账异常处理 | `finance.exception.handle` | 隐藏且不发送请求 |
...
- Path fixes made after backend merge: obsolete Phase 2 sync task/log aliases were retired in favor of `sync-jobs` and `sync-runs`.
  - 阶段2曾缺少 `GET /api/finance/reconciliation/matches/{id}/`；模块化开发合同已新增独立详情端点，旧集合详情兼容说明不再适用。
...
| 库存预警 | `/alerts/inventory` | `/api/internal/alerts/inventory/*` | `alerts.view/evaluate/manage` | `frontend/src/mock/alerts.js`及Sandbox fixture | pending（待模块JWT复验） |
| 补货建议 | `/replenishment/recommendations` | `/api/internal/replenishment/recommendations/*`、`evaluate-mock/` | `replenishment.view/evaluate/review` | `frontend/src/mock/replenishment.js`及Sandbox fixture | pending；evaluate为mock |
| 财务对账 | `/finance/reconciliation/*`、`/finance/analytics` | `/api/finance/analytics/*`、`/api/finance/{statements,withdrawals,bank-receipts}/`、`/api/finance/reconciliation/*` | `finance.view/import/reconcile/exception.handle`，逐action独立scope | `frontend/src/mock/financeReconciliation.js`及Sandbox fixture | pending（单元测试通过，待Local Sandbox JWT复验） |
...
|---|---|---|---|---|---|---|---|
| 平台门店授权列表 | `/integrations/configs` | `/api/internal/integrations/store-authorizations/` | GET | `page`、`page_size`、`platform`、`status`、`store_id` | `count/next/previous/results`；授权元数据与凭据掩码，不返回引用原值 | 空授权列表 Mock | pending |
| 平台门店授权详情 | `/integrations/configs` | `/api/internal/integrations/store-authorizations/{id}/` | GET | `id` | tenant/store scoped 授权元数据，不返回 Token/Secret | 无原始凭据 Mock | pending |
| 平台门店授权动作 | `/integrations/configs` | OAuth start、Shopee/TikTok callback、refresh、revoke | GET/POST | `platform`、内部 config/store、region、redirect；callback 仅平台字段 | `authorization_url` 或脱敏授权元数据；不返回 Token/Secret | pending/mock，不生成真实授权地址 | pending/live-validation |
