...
| 平台生产准入 | `/integrations/readiness` | `/api/internal/integrations/readiness/` | GET | 平台、环境及配置范围 | 准入结论、阻断项、证据摘要 | `frontend/src/mock/integrations.js` | connected |
| 店铺授权列表与动作 | `/integrations/authorizations` | `/api/internal/integrations/store-authorizations/` 及授权、刷新、撤销动作 | GET/POST | 平台、店铺、状态、分页及动作字段 | 授权状态、scope、到期时间、脱敏凭据引用 | `frontend/src/mock/integrations.js` | connected |
| 接入能力矩阵 | `/integrations/capabilities` | `/api/internal/integrations/store-authorizations/{id}/capabilities/` | GET/PUT | 授权 ID、只读能力配置 | 资源、读取开关、同步模式、状态；`write_enabled=false` | `frontend/src/mock/integrations.js` | connected |
| 店铺映射 | `/integrations/store-mappings` | `/api/internal/integrations/store-mappings/`、`{id}/` | GET/POST/PATCH | 平台、店铺、授权、分页及允许修改字段 | 店铺关联、区域、时区、币种、状态 | `frontend/src/api/integrations.js` 脱敏 fixture | connected |
| 商品映射 | `/integrations/product-mappings` | `/api/internal/integrations/product-mappings/`、`{id}/` | GET/POST/PATCH | 平台商品/变体、内部商品/SKU、分页及状态流转字段 | 映射状态、来源、置信度、人工确认状态 | `frontend/src/api/integrations.js` 脱敏 fixture | connected |
| 同步异常处置 | `/integrations/incidents` | `/api/internal/integrations/sync-alert-incidents/` 及详情动作 | GET/POST | 状态及受控处置动作 | 异常详情、状态、审计结果、模拟重试预览 | `frontend/src/mock/integrations.js` | connected |
| 集成审计集合 | `/integrations/audit` | `/api/internal/integrations/audit/` | GET | `page`、`page_size`、`config_id`、`platform`、`action` | 租户与数据范围内的分页脱敏审计记录 | `frontend/src/api/integrations.js` 脱敏 fixture | connected |
