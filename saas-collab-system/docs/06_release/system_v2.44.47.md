...
- 顶级菜单仍为 16 个，名称和顺序保持 V2.44.46 基线。
- 原“基础档案”13 个子节点名称和顺序不变，仅在末尾增加第 14 个节点。
...
- 前端完整测试：253 项通过。
- 前端生产构建：通过。
- 后端基础档案设置、商品 API 与分类元数据定向测试：13 项通过。
- Django `check`：通过。
...
当前只完成 V2.44.47 候选源码和版本登记，未更新虚拟机。正式发布需同时更新前端、后端并执行两项数据库迁移，发布前后均需复核菜单、权限、页面和租户隔离。
