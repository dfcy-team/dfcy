...
case "$mode" in
  ...
  db)
    ...
    if [ "$deployment_mode" = "single-host" ]; then
      ...
      iptables -C "$chain" -s "$app_subnet" -d "$db_ip" -p tcp --dport "$db_port" -j ACCEPT || fail "Single-host application-to-host database allow rule is missing."
      iptables -C "$chain" -s "$app_subnet" -d "$subnet" -p tcp --dport 3306 -j ACCEPT || fail "Single-host application-to-database allow rule is missing."
