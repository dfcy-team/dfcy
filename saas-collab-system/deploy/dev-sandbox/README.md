...
- Local Sandbox 启动时会把 profile 写入 `LOCAL_SANDBOX_MODULE`；profile 同时控制合成数据和菜单模块，未选中的模块显示为关闭，便于本地按模块开发。
- 生产环境不要通过前端直接改进程变量。管理员在“系统管理 → 模块发布控制”中维护 17 个模块；API 网络、平台和同步配置仍在“API 数据接入 → 生产环境配置”维护。模块配置提交版本后由审批人批准；批准后前端菜单、路由、API 同步任务和全球刊登生产队列统一使用该版本。
- 模块状态含“关闭 / 仅 Mock / 试运行只读 / 正式启用”。API 的外部写操作仍由独立的 `sync_write_enabled` 和平台能力矩阵控制，模块启用不会自动开放写入。
- 如需紧急下线，管理员可创建关闭版本并审批；版本历史支持回滚。生产发布仍需遵循 `deploy/production-control` 的架构师审批和不可变镜像流程。
