...
release_dir=/home/dfcy01/releases/system-v2.44.36-build-20260823
...
grep -Fxq 'saas-collab-frontend:v2.44.36' "$release_dir/compose-images.txt"
...
test "$(docker inspect application-frontend-1 --format '{{.Config.Image}}')" = 'saas-collab-frontend:v2.44.36'
...
for path in / /products/master /products/details /influencers /influencers/outreach-tasks /influencers/sample-fulfillments /influencers/bd-performance /system/roles; do
...
for unexpected in 候选款登记 竞品监控 样品/打样管理 比价管理 上架决策 开发档案/转正 首单与试销 返单决策 淘汰库 开发设置; do
  ! docker exec application-frontend-1 grep -Fq "$unexpected" "/usr/share/nginx/html/$main_asset"
done
...
docker exec application-frontend-1 grep -Fq '.mobile-sidebar-drawer' "$layout_css"

docker exec application-frontend-1 sh -c \
  "for f in /usr/share/nginx/html/assets/MainLayout-*.js; do grep -Fq 'Pilot API' \"\$f\"; ! grep -Fq 'stage0_internal_user' \"\$f\"; done"
