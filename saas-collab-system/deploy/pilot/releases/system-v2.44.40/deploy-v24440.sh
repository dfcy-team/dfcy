...
release_dir=/home/dfcy01/releases/system-v2.44.40-build-20260824
...
docker image inspect saas-collab-backend:v2.44.40 >/dev/null
docker image inspect saas-collab-frontend:v2.44.40 >/dev/null
...
grep -Fxq 'saas-collab-backend:v2.44.40' "$release_dir/compose-images.txt"
grep -Fxq 'saas-collab-frontend:v2.44.40' "$release_dir/compose-images.txt"
...
"${compose[@]}" run --rm --no-deps \
  ...
  -e DB_NAME=/tmp/v24440-test.sqlite3 \
  -e DB_USER= \
  -e DB_PASSWORD= \
  -e DB_HOST= \
  -e DB_PORT= \
...
print("TENANT1_FILLED_PLATFORM_PRODUCT_ID={}".format(rows.exclude(platform_product_id="").count()))
print("TENANT1_DISTINCT_PLATFORM_PRODUCT_ID={}".format(rows.exclude(platform_product_id="").values("platform_product_id").distinct().count()))
...
grep -Eq '^TENANT1_FILLED_PLATFORM_PRODUCT_ID=[0-9]+$' "$release_dir/tenant1-before.txt"
grep -Eq '^TENANT1_DISTINCT_PLATFORM_PRODUCT_ID=[0-9]+$' "$release_dir/tenant1-before.txt"
grep -Eq '^TENANT1_VARIANT_DUPLICATE_GROUPS=[0-9]+$' "$release_dir/tenant1-before.txt"
...
MYSQL_PWD="$db_password" mysqldump --no-tablespaces --single-transaction --quick --skip-lock-tables \
  ...
  | gzip -c > "$release_dir/pre-deploy-v2.44.40.sql.gz"
gzip -t "$release_dir/pre-deploy-v2.44.40.sql.gz"
sha256sum "$release_dir/pre-deploy-v2.44.40.sql.gz" > "$release_dir/pre-deploy-v2.44.40.sql.gz.sha256"
...
test "$(docker inspect application-backend-1 --format '{{.Config.Image}}')" = 'saas-collab-backend:v2.44.40'
test "$(docker inspect application-celery-1 --format '{{.Config.Image}}')" = 'saas-collab-backend:v2.44.40'
test "$(docker inspect application-celery-beat-1 --format '{{.Config.Image}}')" = 'saas-collab-backend:v2.44.40'
test "$(docker inspect application-frontend-1 --format '{{.Config.Image}}')" = 'saas-collab-frontend:v2.44.40'
...
for metric in TENANT1_PLATFORM_DETAILS TENANT1_EMPTY_PLATFORM_PRODUCT_ID TENANT1_FILLED_PLATFORM_PRODUCT_ID TENANT1_DISTINCT_PLATFORM_PRODUCT_ID TENANT1_VARIANT_DUPLICATE_GROUPS; do
...
docker exec application-frontend-1 sh -c "grep -R -Fq '导入结果' /usr/share/nginx/html/assets"
docker exec application-frontend-1 sh -c "grep -R -Fq '未匹配' /usr/share/nginx/html/assets"
...
sha256sum "$release_dir/pre-deploy-v2.44.40.sql.gz" > "$release_dir/artifacts.sha256"
