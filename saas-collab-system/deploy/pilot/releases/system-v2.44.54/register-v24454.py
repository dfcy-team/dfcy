...
VERSION = "2.44.54"
PARENT = "2.44.53"
PARENT_COMMIT = "37eaa4a3344be9d3a3c6897e6e4936972a429c40"
COMMIT = "cb022ea3bee8a79e45800632a898afe62d54524a"
TAG = "v2.44.54-deployed"
SCOPE = "masterdata.safe_delete"
...
def fail(message):
    raise SystemExit(f"V2.44.54 registration blocked: {message}")
...
def main():
    ...
    for current, label in ((unified_current, "unified"), (legacy_current, "legacy")):
        if current.get("current_release_version") != PARENT or current.get("git_commit") != PARENT_COMMIT:
            fail(f"{label} ledger no longer matches the V2.44.53 parent")

    expected = {
        "application-backend-1": "saas-collab-backend:v2.44.54",
        "application-celery-1": "saas-collab-backend:v2.44.54",
        "application-celery-beat-1": "saas-collab-backend:v2.44.54",
        "application-frontend-1": "saas-collab-frontend:v2.44.54",
    ...
    for image in ("saas-collab-backend:v2.44.54", "saas-collab-frontend:v2.44.54"):
    ...
    for mirror in mirrors:
        ...
        if mirror_commit(mirror, CANONICAL_REF) != COMMIT:
            fail(f"canonical ref is not V2.44.54 in {mirror}")

    backend_digest = image_id("saas-collab-backend:v2.44.54")
    frontend_digest = image_id("saas-collab-frontend:v2.44.54")
    ...
    release_record = {
        ...
        "git_tag": TAG,
        "actor": "architect",
        "source_actor": "architect",
        "current": True,
        "status_in_unified_ledger": "deployed_pending_owner_verification",
        "scope": SCOPE,
        "canonical_ref": CANONICAL_REF,
        "source_manifest": str(RELEASE_DIR),
        "frontend_image": "saas-collab-frontend:v2.44.54",
        "frontend_digest": frontend_digest,
        "backend_image": "saas-collab-backend:v2.44.54",
        "backend_digest": backend_digest,
        "celery_image": "saas-collab-backend:v2.44.54",
        "celery_beat_image": "saas-collab-backend:v2.44.54",
    ...
    for path in (marker_path, legacy_marker_path, RELEASE_DIR / "OWNER_VERIFICATION_REQUIRED.json"):
    ...
    current_payload = {
        ...
        "canonical_ref": CANONICAL_REF,
        "source_manifest": str(RELEASE_DIR),
        "runtime_frontend_image": "saas-collab-frontend:v2.44.54",
        "runtime_frontend_digest": frontend_digest,
        "runtime_backend_image": "saas-collab-backend:v2.44.54",
        ...
        "runtime_celery_image": "saas-collab-backend:v2.44.54",
        "runtime_celery_beat_image": "saas-collab-backend:v2.44.54",
        "status": "deployed_pending_owner_verification",
    ...
    print(f"V2.44.54 REGISTRATION=PASS status=deployed_pending_owner_verification tag={TAG} scope={SCOPE}")
