...
fail() { echo "V2.44.54 verification failed: $*" >&2; exit 1; }

for container in application-backend-1 application-celery-1 application-celery-beat-1; do
  [ "$(docker inspect "$container" --format '{{.Config.Image}}')" = saas-collab-backend:v2.44.54 ] || fail "$container image mismatch"
done
[ "$(docker inspect application-frontend-1 --format '{{.Config.Image}}')" = saas-collab-frontend:v2.44.54 ] || fail "frontend image mismatch"
[ "$(docker image inspect saas-collab-backend:v2.44.54 --format '{{index .Config.Labels "org.opencontainers.image.revision"}}')" = "$candidate" ] || fail "backend revision mismatch"
[ "$(docker image inspect saas-collab-frontend:v2.44.54 --format '{{index .Config.Labels "org.opencontainers.image.revision"}}')" = "$candidate" ] || fail "frontend revision mismatch"
...
docker exec application-backend-1 python manage.py showmigrations masterdata > "$release_dir/post-masterdata-migrations.txt" 2>&1 || fail "masterdata migrations could not be read"
...
for path in / /influencers /influencers/outreach-tasks /influencers/bd-performance /products/master /products/details /products/platform-details /master-data/platforms /master-data/sites /master-data/stores /master-data/warehouses /master-data/suppliers /system/roles; do
...
api_code=$(curl -ksS -o /dev/null -w '%{http_code}' "https://127.0.0.1:${https_port}/api/internal/master-data/platforms/" || true)
[ "$api_code" = 401 ] || fail "unauthenticated masterdata API returned HTTP $api_code"
printf 'GET /api/internal/master-data/platforms/ %s\n' "$api_code" >> "$release_dir/page-status.txt"
...
echo "V2.44.54 VERIFY=PASS candidate=$candidate migration=NONE"
