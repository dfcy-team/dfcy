"""Root URL configuration for the backend project."""
from django.contrib import admin
from django.urls import include, path


urlpatterns = [
    path("admin/", admin.site.urls),
    path("api/internal/", include("apps.accounts.urls_internal")),
    path("api/miniapp/", include("apps.accounts.urls_miniapp")),
    path("api/miniapp/releases/", include("apps.releases.urls_miniapp")),
    path("api/miniapp/supply-chain/", include("apps.purchasing.urls_miniapp")),
    path("api/miniapp/supply-chain/packing/", include("apps.packing.urls_miniapp")),
    path("api/miniapp/supply-chain/", include("apps.consolidation.urls_miniapp")),
    path("api/internal/integrations/", include("apps.integrations.urls_internal")),
    path("api/internal/analytics/", include("apps.reports.urls_internal")),
    path("api/internal/alerts/", include("apps.alerts.urls")),
    path("api/internal/replenishment/", include("apps.replenishment.urls")),
    path("api/internal/lifecycle/", include("apps.products.urls_lifecycle")),
    path("api/internal/config/", include("apps.configcenter.urls")),
    path("api/internal/master-data/", include("apps.masterdata.urls")),
    path("api/internal/influencers/", include("apps.influencers.urls")),
    path("api/internal/rpa/", include("apps.rpa.urls_internal")),
    path("api/internal/workflow/", include("apps.workflows.urls")),
    path("api/internal/governance/", include("apps.governance.urls")),
    path("api/internal/pilot/", include("apps.pilot.urls")),
    path("api/internal/releases/", include("apps.releases.urls_internal")),
    path("api/internal/sales-management/", include("apps.sales_management.urls")),
    path("api/internal/commerce/", include("apps.sales_management.urls")),
    path("api/internal/products/", include("apps.products.urls")),
    path("api/internal/development/", include("apps.development.urls")),
    path("api/internal/listings/", include("apps.listings.urls")),
    path("api/internal/purchasing/", include("apps.purchasing.urls")),
    path("api/internal/packing/", include("apps.packing.urls_internal")),
    path("api/internal/supply-chain/consolidations/", include("apps.consolidation.urls_internal")),
    path("api/internal/supply-chain/shipments/", include("apps.shipping.urls_internal")),
    path("api/internal/suppliers/", include("apps.suppliers.urls_internal")),
    path("api/external/", include("apps.accounts.urls_external")),
    path("api/external/supplier/purchase-orders/", include("apps.purchasing.urls_supplier")),
    path("api/external/supplier/packing/", include("apps.packing.urls_supplier")),
    path("api/external/supplier/consolidations/", include("apps.consolidation.urls_supplier")),
    path("api/external/supplier/", include("apps.suppliers.urls_external")),
    path("api/rpa/", include("apps.rpa.urls")),
    path("api/platform/", include("apps.integrations.urls_platform")),
    path("api/wechat/", include("apps.integrations.urls_wechat")),
    path("api/feishu/", include("apps.integrations.urls_feishu")),
    path("api/finance/", include("apps.finance.urls")),
    path("api/report/", include("apps.reports.urls")),
]
