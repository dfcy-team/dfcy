...
def identity_collection(request):
    ...
    obj = FeishuIdentity.objects.create(
        tenant=request.user.tenant, user=user, open_id=payload.get("open_id", ""),
        feishu_user_id=payload.get("user_id", ""), union_id=payload.get("union_id", ""),
