"""Approved, expiry-driven renewal of existing Shopee/WMS custody references."""
...
def _platform(record):
    return "shopee" if isinstance(record, MarketplaceStoreAuthorization) else "jifeng_wms"
...
def _actor_allowed(record):
    ...
    if (
        ...
        or actor.tenant_id != record.tenant_id
    ...
    codes = (
        ...
        if warehouse
        else ("integrations.store.authorize", "integrations.credential.rotate")
    )
    return all(
        user_has_integration_permission(actor, code)
        and integration_values_allowed(
            actor,
            code,
...
def refresh_due_authorizations(limit=100):
    ...
    for platform, queryset in sources:
        ...
        for record in queryset.select_related("integration_config", "updated_by").iterator():
            ...
            key = sha256(
                f"{platform}:{record.tenant_id}:{record.pk}:{record.token_id}".encode()
            ).hexdigest()
            attempt, created = AutomaticRefreshAttempt.objects.get_or_create(
                request_key=key,
                defaults={"tenant_id": record.tenant_id},
            )
            if not created:
                continue
            counts["attempted"] += 1
            ...
            try:
                ...
                detail.update(
                    error_code="AUTO_REFRESH_FAILED",
                    reason="自动续期未完成；请检查授权、权限、准入及网络，手动刷新或重新授权后恢复。",
                )
            ...
            IntegrationAuditLog.objects.create(
                ...
                store_authorization=record if platform == "shopee" else None,
                actor=record.updated_by,
