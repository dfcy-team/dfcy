"""Filesystem-backed credential custody kept outside the business database.

The directory and files are the security boundary. Public methods return only
opaque reference metadata; raw values are available only to the internal
runtime adapter through ``_resolve_credentials``.
"""
...
CredentialCustody = FileCredentialStore
CredentialCustodyError = FileCustodyError
CredentialValidationError = FileCustodyValidationError
