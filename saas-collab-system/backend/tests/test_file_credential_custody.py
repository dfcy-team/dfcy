...
def test_file_store_revoke_erases_values_and_is_irreversible(tmp_path):
    ...
    assert store.get_reference(reference["credential_id"])["status"] == "revoked"
...
def test_file_backend_satisfies_runtime_custody_contract(tmp_path):
    ...
    first_access = "unit-" + "access-token"
    first_refresh = "unit-" + "refresh-token"
    ...
    assert backend.retrieve_access_token(first["token_id"]) == first_access
    assert backend.retrieve_refresh_token(first["token_id"]) == first_refresh

    rotated_access = "unit-new-" + "access"
    rotated_refresh = "unit-new-" + "refresh"
    rotated = backend.rotate_secrets(
        previous_credential_id=first["credential_id"],
        previous_token_id=first["token_id"],
        credential_type="shopee",
        reference_version=2,
        app_secret="unit-new-secret",
...
def test_expired_access_is_rejected_but_refresh_remains_available(tmp_path):
    backend = FileCustodyBackend(tmp_path / "credential-custody")
    access_value = "expired-" + "access"
    refresh_value = "usable-" + "refresh"
    ...
    assert backend.retrieve_refresh_token(reference["token_id"]) == refresh_value
