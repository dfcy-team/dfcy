...
def _new_affiliate_order(tenant, *, data_time, product_id="P-1", fully_returned="否", order_status="completed"):
    order = AffiliateOrderSnapshot(
        ...
        source_row_key=affiliate_order_source_row_key(
            ...
            site="PH",
            order_id=f"ORDER-{data_time.timestamp()}",
            sku_id="SKU-1",
        ),
        row_hash="a" * 64,
        data_time=data_time,
...
def test_bd_performance_requires_both_permissions_and_empty_tenant_is_not_imported():
    ...
    assert response.data["data"]["rates"]["PHP"] == "0.1300"
    assert response.data["data"]["rates"]["PHP"] != response.data["data"]["rates"]["USD"]
