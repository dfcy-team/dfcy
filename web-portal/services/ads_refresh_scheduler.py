# -*- coding: utf-8 -*-
"""网站常驻时，按 config/广告定时刷新.ini 在进程内定时刷新广告户。"""
...
def _enabled() -> bool:
    cp = _read_ini()
    val = cp.get("Web服务", "启用内置调度", fallback="0").strip().lower()
...
def _schedule_times() -> list[str]:
    ...
    evening = cp.get("定时", "晚间执行时间", fallback="").strip()
    if morning:
...
def start_if_enabled() -> None:
    ...
    with _lock:
        ...
        print(f"[ads_refresh_scheduler] 已启动内置调度: {times}")
